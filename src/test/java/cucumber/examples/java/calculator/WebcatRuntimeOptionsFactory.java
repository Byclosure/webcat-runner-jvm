...
public class WebcatRuntimeOptionsFactory {
    private final Class clazz;
    private boolean featuresSpecified = false;
    private boolean glueSpecified = false;
    private boolean pluginSpecified = false;

    public WebcatRuntimeOptionsFactory(Class clazz) {
        this.clazz = clazz;
    }

    public RuntimeOptions create() {
        final List<String> args = buildArgsFromOptions(null);
        return new RuntimeOptions(args);
    }

    public RuntimeOptions create(List<String> features) {
        final List<String> args = buildArgsFromOptions(features);
        return new RuntimeOptions(args);
    }

    private List<String> buildArgsFromOptions(List<String> features) {
        final boolean shouldAddAnnotationFeatures = features == null;
        ...
        for (Class classWithOptions = clazz; hasSuperClass(classWithOptions); classWithOptions = classWithOptions.getSuperclass()) {
            ...
            if (options != null) {
                ...
                if(shouldAddAnnotationFeatures) {
                    addFeatures(options, args);
                }
            }
        }

        if(!shouldAddAnnotationFeatures && !features.isEmpty()) {
